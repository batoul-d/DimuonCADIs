          ...
          if (Workspace.count(wsName.Data())>0) {
            ...
            if (doSimulFit) {
              ...
              if (!fitCharmonia( Workspace[DSTAG.Data()], cutVectors[j].at(i), parIniVectors[j].at(i), outputDir,
				 ...
				 applyJEC,        // Flag to indicate if we want to apply the Jet Energy corrections
...
bool setParameters(map<string, string> row, struct KinCuts& cut, map<string, string>& parIni, bool isPbPb, bool doConstrFit)
{
  ...
  for(map<string, string>::iterator col=row.begin(); col!=row.end(); ++col) {
    ...
    if (label=="rap") {
      ...
      if (col->second=="" || col->second.find("-")==std::string::npos) {
	cout << "[ERROR] Input column 'zed' has invalid value: " << col->second << endl; return false;
      }
      std::vector<double> v;
      if(!parseString(col->second, "-", v)) { return false; }
      if (v.size()!=2) {
	cout << "[ERROR] Input column 'zed' has incorrect number of values, it should have 2 values but has: " << v.size() << endl; return false;
      ...
      if (col->second=="" || col->second.find("-")==std::string::npos) {
	cout << "[ERROR] Input column 'jetpt' has invalid value: " << col->second << endl; return false;
      }
      std::vector<double> v;
      if(!parseString(col->second, "-", v)) { return false; }
      if (v.size()!=2) {
	cout << "[ERROR] Input column 'jetpt' has incorrect number of values, it should have 2 values but has: " << v.size() << endl; return false;
      ...
      if (col->second=="" || col->second.find("-")==std::string::npos) {
	cout << "[ERROR] Input column 'jetrap' has invalid value: " << col->second << endl; return false;
      }
      std::vector<double> v;
      if(!parseString(col->second, "-", v)) { return false; }
      if (v.size()!=2) {
...
{ 
  ...
  if (!fitData && strcmp(applyCorr,"")) {
    ...
    cout << "[WARNING] You are applying " << applyCorr << " corrections to MC, If you want to fit Data please make fitData = true." << endl;
  }
  if (!fitData && applyJEC){
    cout << "[WARNING] You are applying Jet Energy corrections on MC. If you want to fit Data please make fitData = true." << endl;
  }
